...
int main(){
	...
	while(flag==0){
		...
		if(C==1){
			obj.sortrn(); obj.showinfo(N); return 0;	
		} else if(C==2){
			obj.sortnm(); obj.showinfo(N); return 0;
		} else if(C==3){
			obj.quicksort(0,N-1); obj.showinfo(10); return 0;
		} else if(C==4){
			obj.srchsgpa(); return 0;
		} else if(C==5){
			obj.binsrch(); return 0; 
		} else{
